...
def recall_list(request):
    queryset = Recall.objects.select_related('supplier', 'created_by').order_by('-recall_date')
...
def recall_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            messages.success(request, f'Recall {recall.document_number} berhasil dibuat.')
            return redirect('recall:recall_detail', pk=recall.pk)
    else:
        form = RecallForm()
        formset = RecallItemFormSet(prefix='items')

    return render(request, 'recall/recall_form.html', {
        'form': form,
        'formset': formset,
...
def recall_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            form.save()
            formset.save()
            messages.success(request, f'Recall {recall.document_number} berhasil diperbarui.')
            return redirect('recall:recall_detail', pk=recall.pk)
    else:
...
def recall_detail(request, pk):
    ...
    return render(request, 'recall/recall_detail.html', {
        'recall': recall,
        'items': items,
    })
...
def recall_verify(request, pk):
    ...
    if recall.status != Recall.Status.SUBMITTED:
        messages.error(request, 'Hanya recall berstatus Diajukan yang dapat diverifikasi.')
        return redirect('recall:recall_detail', pk=pk)
        ...
        with transaction.atomic():
            for recall_item in recall_items:
                ...
                if stock.item_id != recall_item.item_id:
                    raise ValueError(f'Batch stok tidak sesuai untuk item {recall_item.item.nama_barang}.')
            ...
            recall.save(update_fields=['status', 'verified_by', 'verified_at', 'updated_at'])

    except ValueError as exc:
        messages.error(request, str(exc))
        return redirect('recall:recall_detail', pk=pk)

    messages.success(request, f'Recall {recall.document_number} berhasil diverifikasi dan stok diperbarui.')
    return redirect('recall:recall_detail', pk=pk)
...
def recall_complete(request, pk):
    ...
    if recall.status != Recall.Status.VERIFIED:
        messages.error(request, 'Hanya recall terverifikasi yang dapat ditandai selesai.')
        return redirect('recall:recall_detail', pk=pk)
