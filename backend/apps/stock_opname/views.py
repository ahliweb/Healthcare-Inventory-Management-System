...
def opname_list(request):
    ...
    return render(request, 'stock_opname/opname_list.html', {
        'opnames': opnames,
        'search': search,
...
def opname_detail(request, pk):
    ...
    items = (
        opname.items
        .select_related(
            'stock__item', 'stock__item__satuan',
            'stock__location', 'stock__sumber_dana',
        )
        .order_by('stock__location__code', 'stock__item__nama_barang')
    )
    ...
    return render(request, 'stock_opname/opname_detail.html', {
        'opname': opname,
...
def opname_start(request, pk):
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            opname_items = []
            for stock in stocks:
                opname_items.append(StockOpnameItem(
                    stock_opname=opname,
                    stock=stock,
...
def opname_input(request, pk):
    ...
    opname = get_object_or_404(
        StockOpname, pk=pk, status=StockOpname.Status.IN_PROGRESS,
    )
    ...
    return render(request, 'stock_opname/opname_input.html', {
        'opname': opname,
        'items': items,
        'locations': locations,
...
def opname_complete(request, pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'Stock Opname {opname.document_number} telah diselesaikan.')
        return redirect('stock_opname:opname_detail', pk=opname.pk)
...
def opname_print(request, pk):
    ...
    return render(request, 'stock_opname/opname_print.html', {
        'opname': opname,
        'locations': locations.values(),
...
def opname_delete(request, pk):
    ...
    opname = get_object_or_404(
        StockOpname, pk=pk, status__in=[StockOpname.Status.DRAFT, StockOpname.Status.IN_PROGRESS],
    )
    ...
    return render(request, 'stock_opname/opname_confirm_delete.html', {
        'opname': opname,
    })
